# Acme-Ltd
A fictional company (Acme Ltd) that wants to collect financial market data, keep it trustworthy over time, and use it to generate insights, risk views, and recommendations for customers.
...
The platform applies temporal rules for instruments:
...
The jobs read Cassandra using the DataStax Spark Cassandra connector configured by `SPARK_CASSANDRA_CONNECTOR_PACKAGE` (default `com.datastax.spark:spark-cassandra-connector_2.12:3.5.1`).
