...
def mark_instrument_deleted(
    ...
    if current is None:
        raise HTTPException(status_code=404, detail="Instrument not found")
    if repo.is_deleted(instrument_id):
        raise HTTPException(status_code=409, detail="Instrument is already deleted")

    effective_from = valid_from or datetime.now(timezone.utc)
    repo.mark_deleted(instrument_id, valid_from=effective_from)