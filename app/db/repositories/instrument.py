...
class InstrumentRepository(CassandraRepository, WarehouseRepository[FinancialInstrument, UUID]):
    ...
    def find_by_symbol(self, symbol: str) -> FinancialInstrument | None:
        normalized_symbol = symbol.strip().upper()
        matches = [
            instrument
            for instrument in self.find_all()
            if instrument.symbol.strip().upper() == normalized_symbol
        ]
        if not matches:
            return None
        return max(matches, key=lambda instrument: instrument.created_at)
    ...
    def is_deleted(self, partition_key: UUID) -> bool:
        latest_version = self._fetch_one(self._select_latest_version, [partition_key])
        if latest_version is None:
            return False
        return getattr(latest_version, "is_delete_marker", False)
