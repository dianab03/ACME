...
def test_find_by_symbol_returns_latest_case_insensitive_match(mock_session):
    now = datetime.now(timezone.utc)
    older = _make_instrument(symbol="AAPL", created_at=now)
    newer = _make_instrument(symbol="aapl", created_at=now + timedelta(seconds=1))
    rows = []
    for inst in [older, newer]:
        row = MagicMock()
        row.instrument_id = inst.instrument_id
        row.symbol = inst.symbol
        row.instrument_class = inst.instrument_class
    ...
    assert result is not None
    assert result.instrument_id == newer.instrument_id
